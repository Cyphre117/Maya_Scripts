...
r_shoulder_btn  = cmds.iconTextButton(w= 30, h=30, bgc=(1.0, 0.2, 0.2), command="cmds.select('ctrl_R_arm_shoulder')",   ann='ctrl_R_arm_shoulder')
r_elbow_btn     = cmds.iconTextButton(w= 20, h=20, bgc=(1.0, 0.2, 0.2), command="cmds.select('ctrl_R_arm_elbow')",      ann='ctrl_R_arm_elbow')
r_wrist_btn     = cmds.iconTextButton(w= 30, h=30, bgc=(1.0, 0.2, 0.2), command="cmds.select('ctrl_R_arm_wrist')",      ann='ctrl_R_arm_wrist')
r_knee_btn      = cmds.iconTextButton(w= 20, h=20, bgc=(1.0, 0.2, 0.2), command="cmds.select('ctrl_R_leg_knee')",       ann='ctrl_R_leg_knee')
r_ankle_btn     = cmds.iconTextButton(w= 30, h=30, bgc=(1.0, 0.2, 0.2), command="cmds.select('ctrl_R_leg_ankle')",      ann='ctrl_R_leg_ankle')
# Create the left controls
l_shoulder_btn  = cmds.iconTextButton(w= 30, h=30, bgc=(0.2, 0.2, 1.0), command="cmds.select('ctrl_L_arm_shoulder')",   ann='ctrl_L_arm_shoulder')
l_elbow_btn     = cmds.iconTextButton(w= 20, h=20, bgc=(0.2, 0.2, 1.0), command="cmds.select('ctrl_L_arm_elbow')",      ann='ctrl_L_arm_elbow')
l_wrist_btn     = cmds.iconTextButton(w= 30, h=30, bgc=(0.2, 0.2, 1.0), command="cmds.select('ctrl_L_arm_wrist')",      ann='ctrl_L_arm_wrist')
l_knee_btn      = cmds.iconTextButton(w= 20, h=20, bgc=(0.2, 0.2, 1.0), command="cmds.select('ctrl_L_leg_knee')",       ann='ctrl_L_leg_knee')
l_leg_btn       = cmds.iconTextButton(w= 30, h=30, bgc=(0.2, 0.2, 1.0), command="cmds.select('ctrl_L_leg_ankle')",      ann='ctrl_L_leg_ankle')
# Create the mid controls
head_btn        = cmds.iconTextButton(w= 40, h=40, bgc=(1.0, 0.7, 0.0), command="cmds.select('ctrl_body_head')",    ann='ctrl_body_head')
jaw_btn         = cmds.iconTextButton(w= 24, h=12, bgc=(1.0, 0.7, 0.0), command="cmds.select('ctrl_body_jaw')",      ann='ctrl_body_jaw')
spine_top_btn   = cmds.iconTextButton(w= 60, h=10, bgc=(1.0, 0.7, 0.0), command="cmds.select('ctrl_spine_top')",    ann='ctrl_spine_top')
spine_mid_btn   = cmds.iconTextButton(w= 50, h=10, bgc=(1.0, 0.7, 0.0), command="cmds.select('ctrl_spine_mid')",    ann='ctrl_spine_mid')
spine_bot_btn   = cmds.iconTextButton(w= 70, h=12, bgc=(1.0, 0.7, 0.0), command="cmds.select('ctrl_spine_bot')",    ann='ctrl_spine_bot')
root_btn        = cmds.iconTextButton(w= 90, h=12, bgc=(1.0, 0.4, 0.0), command="cmds.select('ctrl_body_root')",    ann='ctrl_body_root')
master_btn      = cmds.iconTextButton(w=200, h=12, bgc=(0.8, 0.2, 0.8), command="cmds.select('ctrl_master')",       ann='ctrl_master')
...
cmds.formLayout(model_tab, e=True, af=[[r_knee_btn,     "right",WIDTH()/2+30], [r_knee_btn,     "bottom", 160]])
cmds.formLayout(model_tab, e=True, af=[[r_ankle_btn,    "right",WIDTH()/2+40], [r_ankle_btn,    "bottom", 100]])
...
cmds.formLayout(model_tab, e=True, af=[[l_knee_btn,     "left", WIDTH()/2+30], [l_knee_btn,     "bottom", 160]])
cmds.formLayout(model_tab, e=True, af=[[l_leg_btn,      "left", WIDTH()/2+40], [l_leg_btn,      "bottom", 100]])
# Position the spine controls
cmds.formLayout(model_tab, e=True, af=[[head_btn,       "left", WIDTH()/2-20 ],[head_btn,       "top",      70]])
cmds.formLayout(model_tab, e=True, af=[[jaw_btn,        "left", WIDTH()/2-12 ],[jaw_btn,        "top",      120]])
cmds.formLayout(model_tab, e=True, af=[[spine_top_btn,  "left", WIDTH()/2-30 ],[spine_top_btn,  "top",      150]])
cmds.formLayout(model_tab, e=True, af=[[spine_mid_btn,  "left", WIDTH()/2-25 ],[spine_mid_btn,  "bottom",   HEIGHT()/2+30]])
cmds.formLayout(model_tab, e=True, af=[[spine_bot_btn,  "left", WIDTH()/2-35 ],[spine_bot_btn,  "bottom",   HEIGHT()/2-10]])
cmds.formLayout(model_tab, e=True, af=[[root_btn,       "left", WIDTH()/2-45 ],[root_btn,       "top",      HEIGHT()/2]])
cmds.formLayout(model_tab, e=True, af=[[master_btn,     "left", WIDTH()/2-100],[master_btn,     "bottom",   80]])
